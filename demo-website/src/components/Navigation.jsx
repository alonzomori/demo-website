...
function Navigation() {
    ...
    return (
        <nav className="mx-auto py-[30px] bg-[#C0C0C0] fixed top-0 w-145 md:w-300 lg:w-390 z-[102]">
            ...
            {open && (
                <div className="w-screen md:hidden bg-white shadow-lg py-4 absolute right-0 top-20 mr-6 rounded-lg">
                    <ul className="flex flex-col space-y-4 px-6 text-right">
                    <Link to="about" smooth={true} duration={600} className="cursor-pointer text-center border-b-1 text-gray hover:text-gray-100"><strong>About</strong></Link>
                    <Link to="services" smooth={true} duration={600} className="cursor-pointer text-center border-b-1 text-gray hover:text-gray-400"><strong>Services</strong></Link>
                    <Link to="contact" smooth={true} duration={600} className="cursor-pointer text-center border-b-1 text-gray hover:text-gray-400"><strong>Contact</strong></Link>  
                    </ul>
                </div>
            )}
        </nav>
